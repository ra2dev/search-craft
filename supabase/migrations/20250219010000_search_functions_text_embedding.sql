...
--
-- NOTE: p_query_embedding is declared `text` so PostgREST (Supabase RPC) can
-- accept the pgvector text form ("[0.1,0.2,...]") from JSON. It is cast to the
-- dimension-typed vector inside the function body.

-- Drop any previous vector-typed overloads so PostgREST doesn't see ambiguous
-- matches after this migration is applied.
...
as $$
  with q as (
    select p_query_embedding::vector(384) as embedding
  ),
  vec as (
    select sd.id, sd.document_id, sd.content, sd.description,
           1 - (sd.embedding_384 <=> q.embedding) as vector_similarity,
           row_number() over (order by sd.embedding_384 <=> q.embedding) as rnk
    from public.search_documents sd, q
    where sd.search_dataset_id = p_search_dataset_id
      and sd.embedding_384 is not null
    order by sd.embedding_384 <=> q.embedding
...
as $$
  with q as (
    select p_query_embedding::vector(768) as embedding
  ),
  vec as (
    select sd.id, sd.document_id, sd.content, sd.description,
           1 - (sd.embedding_768 <=> q.embedding) as vector_similarity,
           row_number() over (order by sd.embedding_768 <=> q.embedding) as rnk
    from public.search_documents sd, q
    where sd.search_dataset_id = p_search_dataset_id
      and sd.embedding_768 is not null
    order by sd.embedding_768 <=> q.embedding
    limit greatest(p_k * 5, 50)
  ),
  fts as (
    select id, document_id, content, description,
...
as $$
  with q as (
    select p_query_embedding::vector(1536) as embedding
  ),
  vec as (
    select sd.id, sd.document_id, sd.content, sd.description,
           1 - (sd.embedding_1536 <=> q.embedding) as vector_similarity,
           row_number() over (order by sd.embedding_1536 <=> q.embedding) as rnk
    from public.search_documents sd, q
    where sd.search_dataset_id = p_search_dataset_id
      and sd.embedding_1536 is not null
    order by sd.embedding_1536 <=> q.embedding
    limit greatest(p_k * 5, 50)
  ),
  fts as (
    select id, document_id, content, description,
...
as $$
  ...
  vec as (
    select sd.id, sd.document_id, sd.content, sd.description,
           1 - (sd.embedding_3072 <=> q.embedding) as vector_similarity,
           row_number() over (order by sd.embedding_3072 <=> q.embedding) as rnk
    from public.search_documents sd, q
    where sd.search_dataset_id = p_search_dataset_id
      and sd.embedding_3072 is not null
    order by sd.embedding_3072 <=> q.embedding
    limit greatest(p_k * 5, 50)
  ),
  fts as (
    select id, document_id, content, description,
...
-- Refresh PostgREST schema cache so the new function signatures are picked up
